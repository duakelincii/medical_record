...
class RMController extends Controller
{
    ...
    {
        DB::table('rm')->where('id',$id)->delete();
    ...
    public function tagihan($id)
    {
        ...
        $record = DB::table('rm')->find($id);

        return view('tagihan',compact('metadatas','idens','items','record'));
    ...
    public function tagihan_export($id)
    {
        $metadatas = ambil_satudata('metadata',14);
        $datas= ambil_satudata('rm',$id);
        foreach ($datas as $data) {
        ...
        $pdf = PDF::loadView('export.tagihan', compact('metadatas', 'idens', 'items', 'record'))->setPaper('a4','landscape');
        return $pdf->stream();
